...
class _BuycommercialState extends State<Buycommercial> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      await _firestoreService.Buycommercial(
            ...
            child: ListView(
              children: [
                ...
                DropdownButtonFormField(
                  ...
                  isExpanded: true,
                  hint: Text('Select property Tax'),
                  items: selectTaxList.map((option) {
                    return DropdownMenuItem(
                      value: option,
                      child: Text(option, style: TextStyle(fontSize: 14)),
                    );
 ...
 /*Widget _buildUploadSection(String title, Function() onTap, String? fileName) {
  ...
  Future<void> _pickImage() async {
    ...
    if (result != null) {
      setState(() {
        uploadedImageName.addAll(result.files.map((file) => file.path!).toList());
      });
    }
  }
